...
class ProjectforkViewReply extends JViewLegacy
{
    protected $form;
    protected $item;
    protected $state;
    ...
    protected function addToolbar()
    {
        ...
        $access      = ProjectforkHelperAccess::getActions('reply', $this->item->id);
        ...
        if ($is_new && $this->state->get('topic_id')) {
            ...
            if (!$checked_out) {
                if ($access->get('reply.edit') || ($access->get('reply.edit.own') && $this->item->created_by == $uid)) {
                    JToolBarHelper::apply('reply.apply');
                    JToolBarHelper::save('reply.save');
                    JToolBarHelper::save2new('reply.save2new');
                }

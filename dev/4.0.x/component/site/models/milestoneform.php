...
require_once JPATH_ADMINISTRATOR.DS.'components'.DS.'com_projectfork'.DS.'models'.DS.'milestone.php';
...
class ProjectforkModelMilestoneForm extends ProjectforkModelMilestone
{
	...
	public function getItem($itemId = null)
	{
		...
		$itemId = (int) (!empty($itemId)) ? $itemId : $this->getState('milestone.id');
		...
		$userId	= $user->get('id');
		$asset	= 'com_projectfork.milestone.'.$value->id;
		...
		if ($itemId) {
			// Existing item
			$value->params->set('access-change', ($user->authorise('core.edit.state', $asset) || $user->authorise('milestone.edit.state', $asset)));
		}
		else {
		    // New item
			$value->params->set('access-change', ($user->authorise('core.edit.state', 'com_projectfork') ||
                                                  $user->authorise('milestone.edit.state', 'com_projectfork')));
		}

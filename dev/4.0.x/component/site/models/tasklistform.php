...
class ProjectforkModelTasklistForm extends ProjectforkModelTasklist
{
	...
	public function getItem($itemId = null)
	{
		...
		$itemId = (int) (!empty($itemId)) ? $itemId : $this->getState('tasklist.id');
		...
		$userId	= $user->get('id');
		$asset	= 'com_projectfork.tasklist.'.$value->id;
		...
		elseif (!empty($userId) && ($user->authorise('core.edit.own', $asset) || $user->authorise('tasklist.edit.own', $asset))) {
		...
		if ($itemId) {
			// Existing item
			$value->params->set('access-change', ($user->authorise('core.edit.state', $asset) || $user->authorise('tasklist.edit.state', $asset)));
		}
		else {
		    // New item
			$value->params->set('access-change', ($user->authorise('core.edit.state', 'com_projectfork') ||
                                                  $user->authorise('tasklist.edit.state', 'com_projectfork')));
		}

...
require_once JPATH_ADMINISTRATOR.DS.'components'.DS.'com_projectfork'.DS.'models'.DS.'project.php';
...
class ProjectforkModelProjectForm extends ProjectforkModelProject
{
	...
	public function getItem($itemId = null)
	{
		...
		$userId	= $user->get('id');
		$asset	= 'com_projectfork.project.'.$value->id;
		...
		if ($itemId) {
			// Existing item
			$value->params->set('access-change', ($user->authorise('core.edit.state', $asset) || $user->authorise('project.edit.state', $asset)));
		}
		else {
		    // New item
			$value->params->set('access-change', ($user->authorise('core.edit.state', 'com_projectfork') ||
                                                  $user->authorise('project.edit.state', 'com_projectfork')));
		}

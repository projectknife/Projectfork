...
class ProjectforkViewNote extends JViewLegacy
{
	...
	function display($tpl = null)
	{
		...
		$results = $dispatcher->trigger('onContentPrepare', array ('com_projectfork.note', &$this->item, &$this->params, $offset));

		$this->item->event = new stdClass();
		$results = $dispatcher->trigger('onContentAfterTitle', array('com_projectfork.note', &$this->item, &$this->params, $offset));
		$this->item->event->afterDisplayTitle = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentBeforeDisplay', array('com_projectfork.note', &$this->item, &$this->params, $offset));
		$this->item->event->beforeDisplayContent = trim(implode("\n", $results));

		$results = $dispatcher->trigger('onContentAfterDisplay', array('com_projectfork.note', &$this->item, &$this->params, $offset));
	...
	protected function _prepareDocument()
	{
		...
		if($menu && ($menu->query['option'] != 'com_projectfork' || $menu->query['view'] != 'task' || $id != $this->item->id)) {

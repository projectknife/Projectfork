...
class PFrepoModelNotes extends JModelList
{
    ...
    public function getListQuery()
    {
        ...
        $query->join('LEFT', '#__pf_ref_labels AS lbl ON (lbl.item_id = a.id AND lbl.item_type = ' . $db->quote('com_pfrepo.note') . ')');
    ...
    public function getItems()
    {
        ...
        foreach ($items as $i => &$item)
        {
            ...
            if ($items[$i]->label_count > 0) {
                $items[$i]->labels = $labels->getConnections('com_pfrepo.note', $items[$i]->id);
            }
        }
